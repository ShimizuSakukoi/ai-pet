...
function showSetup(isEdit) {
    ...
    if (isEdit && savedSettings && savedSettings.model_name) {
        ...
        for (let i = 0; i < sel.options.length; i++) {
            if (sel.options[i].value === savedSettings.model_name) { sel.value = savedSettings.model_name; found = true; break; }
        }
        if (!found) {
            const o = document.createElement("option");
            o.value = savedSettings.model_name; o.textContent = savedSettings.model_name; o.selected = true;
            sel.appendChild(o);
        }
    }
}
